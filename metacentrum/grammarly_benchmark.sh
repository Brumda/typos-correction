#!/bin/bash
#PBS -N Grammarly-benchmark-prefixes
#PBS -l walltime=48:0:0
#PBS -l select=1:ncpus=1:ngpus=1:mem=32gb:scratch_local=80gb:cluster=adan
#PBS -m abe
#PBS -j oe
...
if [[ "$model" == *finetuned* ]]; then
    ...
    python grammarly_benchmark.py --prefix="$prefix" --finetuned || { echo >&2 "Python script failed"; exit 1; }
else
    echo "Pre-trained model"
    python grammarly_benchmark.py --prefix="$prefix" || { echo >&2 "Python script failed"; exit 1; }
fi

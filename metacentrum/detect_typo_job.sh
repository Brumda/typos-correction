...
#PBS -l select=1:ncpus=1:ngpus=1:mem=20gb:scratch_local=100gb
#PBS -m abe
#PBS -j oe
...
cp "$SCRATCHDIR/$PROJECT_NAME/detect_typo_result.txt" "$DATADIR/../detect_typo/result_$(date '+%Y_%m_%d_%H').txt"  || { echo >&2 "Failed to results"; exit 1; }

...
cp "$SCRATCHDIR/$PROJECT_NAME/benchmark_results.txt" "$DATADIR/../benchmark_results/elmo-finetuned-space_correction-$(date '+%Y_%m_%d_%H').txt" || { echo >&2 "Failed to results"; exit 1; }

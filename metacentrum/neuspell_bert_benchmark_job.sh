#!/bin/bash
#PBS -N BERT_Benchmark-finetuned-TOK
...
python neuspell_benchmark.py --finetuned --tokenize --no_fix_spaces || { echo >&2 "Python script failed"; exit 1; }

cp "$SCRATCHDIR/$PROJECT_NAME/benchmark_results.txt" "$DATADIR/../benchmark_results/bert_finetuned_wo_space_correction_toks$(date '+%Y_%m_%d_%H').txt"  || { echo >&2 "Failed to results"; exit 1; }

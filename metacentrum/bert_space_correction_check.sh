#!/bin/bash
#PBS -N BERT_space_correction_check
...
python bert_space_correction_check.py || { echo >&2 "Python script failed"; exit 1; }

cp "$SCRATCHDIR/$PROJECT_NAME/results.txt" "$DATADIR/../space_correction_examples-$(date '+%Y_%m_%d_%H').txt"  || { echo >&2 "Failed to results"; exit 1; }
